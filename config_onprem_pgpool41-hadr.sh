...
if [ ! -f ""$PARMFILE"" ]; then
   echo '#DB/Proxy nodes...node 0-2 are local cluster nodes, 3 is remote DB DR node' > "$PARMFILE"
   echo '#nodeid:hostname_or_ip[:nodb][dr]' >> "$PARMFILE"
   echo '0:' >> "$PARMFILE"
   echo '1:' >> "$PARMFILE"
   echo '2:' >> "$PARMFILE"
   echo '#3::dr' >> "$PARMFILE"
   ...
   sed -i 's/^#nodeid:hostname_or_ip$/#nodeid:hostname_or_ip[:nodb|:dr]/' "$PARMFILE"
fi
...
# Check that the local cluster db/pgpool hosts are on the same subnet in the local cluster...abort with error if not.
SUBNET0=`ssh -q "$PARM_HOSTNAME0" "hostname -i" | cut -d'.' -f1,2,3`; echo "$PARM_HOSTNAME0" in subnet "$SUBNET0"
SUBNET1=`ssh -q "$PARM_HOSTNAME1" "hostname -i" | cut -d'.' -f1,2,3`; echo "$PARM_HOSTNAME1" in subnet "$SUBNET1" 
SUBNET2=`ssh -q "$PARM_HOSTNAME2" "hostname -i" | cut -d'.' -f1,2,3`; echo "$PARM_HOSTNAME2" in subnet "$SUBNET2"
MYHOSTNAME=`hostname -s`

if [ -n "$SUBNET0" -a -n "SUBNET1" -a -n "SUBNET2" -a "$SUBNET0" == "$SUBNET1" -a "$SUBNET0" == "$SUBNET2" -a "$SUBNET1" == "$SUBNET2" ]; then
   echo "DB/PGPool host subnets match...proceeding."
else
   echo "DB/PGPool host subnets do NOT match. Virtual IP's cannot span subnets...re-IP hosts to same subnet." 
   exit 1 
fi
...
# Postgres DB + PGPool node 0 - 2...node 0 will always be default DB primary
...
if [ -n "$PARM_HOSTNAME0" -a -z "$PARM_OPTION0" ]; then echo "backend_application_name0 = '$PARM_HOSTNAME0'"   >> "$CFGFILE"; fi
...
if [ -n "$PARM_HOSTNAME1" -a -z "$PARM_OPTION1" ]; then echo "backend_application_name1 = '$PARM_HOSTNAME1'"   >> "$CFGFILE"; fi
...
if [ -n "$PARM_HOSTNAME2" -a -z "$PARM_OPTION2" ]; then echo "backend_application_name2 = '$PARM_HOSTNAME2'"   >> "$CFGFILE"; fi

# Postgres DB Remote DR node 3 (failover to remote DR DB is disabled in failover process rather than here)
